        ...
        <div class="myPageTop45">
            <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch">
                ...
                <el-select
                    ...
                    class="selfSerchInput marginRight10"
                >
                    <el-option v-for="(dict, k) in sys_oper_type" :key="k" :label="dict.label" :value="k" />
                </el-select>
                <el-select
                    ...
                    class="selfSerchInput marginRight10"
                >
                    <el-option v-for="(dict, k) in sys_common_status" :key="k" :label="dict.label" :value="k" />
                </el-select>
                <el-date-picker
                    v-model="dateRange"
            ...
            >
                ...
                <el-table-column label="操作类型" align="center" prop="businessType">
                    <template #default="scope">
                        <span>{{ sys_oper_type[scope.row.businessType].dictLabel }}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作状态" align="center" prop="status">
                    <template #default="scope">
                        <span>{{ sys_common_status[scope.row.status].dictLabel }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                    label="操作日期"
...
export default {
    ...
    created() {
        this.getList();
        this.getDict('sys_common_status').then((res) => {
            this.sys_common_status = res;
        });
        this.getDict('sys_oper_type').then((res) => {
            this.sys_oper_type = res;
        });
    },
    methods: {
        /** 查询登录日志 */
        ...
        handleDelete(row) {
            ...
            this.$confirm('是否确认删除日志编号为"' + operIds + '"的数据项？')
                .then(function () {
