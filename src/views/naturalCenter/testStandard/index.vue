    ...
    <div class="myPage">
        <div class="myPageTop45">
            <el-form :model="queryParams" ref="queryForm" :inline="true">
                ...
                <el-select
                    ...
                    clearable
                >
                    <el-option v-for="item in legalEffectLS" :key="item" :label="item" :value="item"> </el-option>
                </el-select>
        ...
        <div class="myCenter90">
            <el-table :data="lis" height="100%" border v-loading="loading" size="small">
                ...
                <el-table-column label="检测类别" prop="testCategory">
                ...
                <el-table-column label="操作" width="100">
                    <template #default="scope">
                        ...
                        <el-popconfirm title="确认删除此标准？" @confirm="delItem(scope.row)">
                            <template #reference>
        ...
        <el-dialog :title="is == 'add' ? '新增法律法规' : '编辑法律法规'" v-model="isShow" width="60%" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" v-if="form != null">
                    ...
                    <el-col :span="12">
                        <el-form-item label="法律效力" prop="legalEffect">
                            <el-select v-model="form.legalEffect" placeholder="法律效力" class="maxWidth" clearable>
                                <el-option v-for="item in legalEffectLS" :key="item" :label="item" :value="item">
                                </el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                ...
                <el-row>
                    ...
                    <el-col :span="12">
                        <el-form-item label="检测类别" prop="testCategory">
                            <el-select v-model="form.testCategory" placeholder="检测类别" class="maxWidth" clearable>
                                <el-option
                                    v-for="(item, k) in testCates"
                                    :key="Number(k)"
                                    :label="item"
                                    :value="Number(k)"
                                >
...
export default {
    data() {
        var implementationDate = (rule, value, callback) => {
            var time = new Date(new Date().toLocaleDateString()).getTime() + 28800000; //当前时间
            var vtime = new Date(value).getTime(); //实施时间
            var abolitionDate = new Date(this.form.abolitionDate).getTime(); //废止时间
            if (this.form.status == 1) {
                if (this.form.abolitionDate) {
                    if (abolitionDate <= vtime) {
                        callback(new Error('实施日期不可大于废止日期日期'));
                    } else {
                        if (vtime <= time) {
                            return callback();
                        } else {
                            callback(new Error('实施日期不可大于当前日期'));
                        }
                    }
                } else {
                    if (vtime <= time) {
                ...
                if (time >= abolitionDate) {
                    return callback();
                } else {
                    callback(new Error('废止日期不可大于当前日期'));
                }
            } else if (this.form.status == 2) {
                if (time <= vtime) {
                    return callback();
                } else {
                    callback(new Error('当前日期不可大于实施日期'));
                }
            }
        };
        ...
        return {
            ...
            rules: {
                ...
                testCategory: [{ required: true, message: '请选择检测类别', trigger: 'change' }],
                implementationDate: [{ required: true, validator: implementationDate, trigger: 'blur' }],
